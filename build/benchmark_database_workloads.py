...
from DITWorkstation.Models import ChecksumAlgorithm, MediaAsset
...
from DITWorkstation.Services.media_import_service import (
    MediaImportService,
)
...
def _make_assets(project_id: str, start: int, count: int) -> list[MediaAsset]:
    ...
    imported_at = datetime(2026, 1, 1, tzinfo=UTC) + timedelta(seconds=start)
    assets = []
...
def _seed(db: DatabaseService, asset_count: int) -> tuple[str, int]:
    ...
    for start in range(0, asset_count, SEED_BATCH_SIZE):
        assets = _make_assets(
            project.project_id, start, min(SEED_BATCH_SIZE, asset_count - start)
        )
        inserted = db.add_media_assets_batch(assets)
        if inserted != len(assets):
            raise RuntimeError(
                f"expected {len(assets)} inserted assets, got {inserted}"
            )
...
def _write_markdown(result: dict, output_path: Path) -> None:
    ...
    for metric in result["measurements"]:
        lines.append(
            ...
            "{peak_python_memory_mb:.2f} | {peak_process_memory_mb:.2f} |".format(
                **metric
            )
        )
    lines.extend(
...
def _run_file_workloads(
    db: DatabaseService, run_dir: Path, asset_count: int
) -> list[Measurement]:
    ...
    def create_fixture():
        ...
        for index in range(asset_count):
            path = (
                media_root
                / f"CARD_{index % 8:02d}"
                / f"REEL_{index // 250:04d}"
                / f"CAM_{index:08d}.cr3"
            )
    ...
    def verify_sample():
        for path in sample_paths:
            checksum = checksum_service.compute_file_checksum(
                path, ChecksumAlgorithm.XXHASH64
            )
            if not checksum_service.verify_file(
                path, checksum.hash_value, ChecksumAlgorithm.XXHASH64
            ):
    ...
    measurements.append(
        Measurement(
            name="import_cancellation_latency",
            elapsed_ms=round(elapsed, 2),
            peak_python_memory_mb=0.0,
            peak_process_memory_mb=_process_memory_mb(),
...
def _run_one(
    asset_count: int, output_dir: Path, keep_database: bool, file_workloads: bool
) -> dict:
    run_dir = Path(
        tempfile.mkdtemp(prefix=f"dit-benchmark-{asset_count}-", dir=output_dir)
    )
    ...
    try:
        ...
        assets, measurement = _measure(
            "get_media_assets_full",
            lambda: (db.get_media_assets(project_id), asset_count),
        )
        measurements.append(measurement)
        ...
        first_page, measurement = _measure(
            "search_first_page",
            lambda: (
                db.search_assets(project_id=project_id, limit=PAGE_SIZE),
                PAGE_SIZE,
            ),
        )
        measurements.append(measurement)
        ...
        deep_page, measurement = _measure(
            "search_deep_page_offset",
            lambda: (
                db.search_assets(
                    project_id=project_id, limit=PAGE_SIZE, offset=deep_offset
                ),
                PAGE_SIZE,
            ),
        )
        ...
        _csv, measurement = _measure(
            "csv_export_streaming",
            lambda: (
                report_service.export_assets_csv_iter(
                    db.iter_search_assets(project_id=project_id, batch_size=PAGE_SIZE),
                    csv_path,
                ),
                asset_count,
            ),
...
def _parse_counts(value: str) -> tuple[int, ...]:
    try:
        ...
        raise argparse.ArgumentTypeError(
            "--assets must be comma-separated positive integers"
        ) from exc
    if not counts or any(count <= 0 for count in counts):
        raise argparse.ArgumentTypeError(
            "--assets must contain at least one positive integer"
        )
    return counts
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run reproducible DIT database benchmarks."
    )
    parser.add_argument(
        "--assets",
        type=_parse_counts,

...
| 6 | 拍摄日志 | 📋 | 管理场景/镜头/镜次拍摄记录，支持「从代表素材填充 EXIF」，双向关联素材与日志；支持 CSV 导入导出（标准镜头清单，按场景/镜头/镜次去重更新） |
| 7 | 素材检索 | 🔍 | 按项目/场景/镜头/类型/关键词/日志/评级/日期/标签多维度组合检索（分页展示，每页 500 条），支持按拍摄日期时间线聚合并下钻到当天列表，结果可导出 CSV；支持按校验和跨项目查重；支持保存搜索/智能集合（命名条件、快速套用、管理对话框） |
| 8 | 素材信息 | ℹ️ | 查看素材 EXIF 与元数据详情，支持缩略图预览、单个/批量重新读取 EXIF，可设置镜次评级，可一键导出项目素材清单 CSV；支持标签/备注编辑、批量评级、软删除、丢失文件检测与重新链接；标签/备注同步写入 XMP sidecar |
...
- 极简模式为 3 页：欢迎 → 创建项目 → 完成（文案仅提示导入链路）
- 完成后自动跳转到「媒体导入」视图

### 使用场景（团队 / 个人 / 极简模式）
- 「设置 → 使用场景」可切换团队模式（默认，完整 9 模块工作流）、个人模式（独立创作者）与极简模式（仅媒体导入）
...
- 极简模式仅保留「媒体导入」一项导航，其余视图不实例化、菜单入口与快捷键同步隐藏；提供「媒体保存目录」自定义路径设置项，导入的素材复制到 `<保存目录>/<项目名>/`，支持团队/个人/极简三种模式随时切回
- 不删除任何数据库数据，三种模式共享同一数据库格式；切换写入 `settings.json` 的 `app_config.usage_mode`，保存目录写入 `app_config.minimal_import_target_dir`，重启后生效

### 极简模式（仅媒体导入）
- 适用场景：只需要「把存储卡素材汇总到本机某个目录」的轻量使用者
- 启动即进入「媒体导入」：选择源目录 → 扫描 → 勾选文件 → 开始导入
- 「保存位置」支持自定义目标目录（选择 / 打开 / 清除），路径持久化到 `settings.json`，重启后保留
- 默认勾选「复制到保存目录」，素材复制到 `<保存目录>/<项目名>/`；同名文件自动追加序号，不覆盖已有文件；取消勾选则按引用方式导入（原文件保持不动）
- 未设置保存目录时导入会即时提示选择，不会静默失败；设置项同时出现在「设置 → 使用场景」与导入视图
- 首启向导精简为「欢迎 → 创建项目 → 完成」，仅提示导入这一条链路
...
### 数据库迁移版本化
- 迁移机制使用 `PRAGMA user_version` 版本化管理（v1 素材字段补齐；v2 标签回填与项目模板；v3 FTS5 索引版本；v4 备份方案模板；v5 回收站、重命名历史与审计字段；v6 任务历史与备份快照；v7 容量趋势快照；v8 拍摄日期时间线索引；v9 跨会话校验和缓存；v10 保存搜索/智能集合），升级路径明确、幂等可重复执行
...
### 全局命令面板（Ctrl+K）
- 按 `Ctrl+K` 打开全局命令面板，类似 Windows 运行对话框（Win+R）。
- 快速导航跳转：输入名称搜索工作区/项目/导航页。
- 直接运行路径/URL：输入文件路径或 URL 按 Enter 即可在文件管理器/浏览器中打开。
- 快速命令入口：打开文件管理器、日志目录、设置。

### 保存搜索与智能集合
- 在素材检索视图可把当前检索条件保存为命名搜索，支持重命名、管理、快速套用。
- 保存搜索可关联工作区/项目范围，不同项目间条件不串扰。
- 上限可在 `AppConfig.saved_search_limit` 中配置（默认 20）。

### 完整性校验调度
- 在「设置 → 完整性校验」中可开启定期自动校验，设定间隔与范围。
- 应用启动后按配置自动执行，校验结果写入操作审计日志。
- 可通过菜单「设置 → 立即完整性校验…」手动触发。

### 自动更新检查
- 在「设置 → 自动更新」中配置更新清单 URL，启动时静默检查。
- 发现新版本时状态栏提示，支持一键打开下载页面。
- 后台线程检查，不阻塞启动和操作。

### 深浅主题切换
- 在「设置 → 外观」中可选择浅色或深色主题。
- 切换后重启应用生效，所有视图（导航栏、表格、按钮、输入框）统一按所选主题配色。

### 多卡去重
- 勾选「跳过已处理过的存储卡」后，已处理过的卡（按内容指纹识别）插入时不会重复触发自动化流程。
- 指纹基于顶层媒体文件名 + 关键目录的 SHA-256 哈希，插拔不同卡或卡内容变化时自动重新识别。

### XMP Sidecar 写入
- 在素材信息视图设置评级/标签/备注时，同步写入 XMP sidecar 文件（`.xmp` 同目录侧挂），Lightroom/Capture One 可直接读取。

### 从备份恢复素材（回拷）
- 通过「设置 → 从备份恢复素材…」打开恢复向导，选择项目/备份位置/目标目录，把已备份素材从备份卷复制回工作盘。
- 支持校验和比对验证，目标已存在且一致时自动跳过。

### 拍摄日志 CSV 导入导出
- 导出：把项目拍摄日志导出为标准镜头清单 CSV，Excel 可直接打开。
- 导入：从 CSV 场记单导入拍摄日志，按「场景+镜头+镜次」自动去重更新。

### 数据库迁移 v10
- 新增 `saved_searches` 表（保存搜索/智能集合持久化），支持按名称/更新时间排序检索。

...
class TestIntegrityScheduler:
    def test_run_verification_writes_audit(self, db_service, project):
        ...
        integrity = IntegrityScheduler(
            None, db_service, scheduler=ScheduledTaskService(tick_seconds=9999)
        )
...
class TestBackupRestore:
    ...
    def test_restore_skips_identical(self, db_service, tmp_dir):
        ...
        hv = checksum.compute_file_checksum(
            str(src_file), ChecksumAlgorithm.XXHASH64
        ).hash_value
...
class TestSavedSearch:
    def test_crud_roundtrip(self, db_service, project):
        s = db_service.create_saved_search(
            "优选集",
            {"rating": 3, "tag": "优选"},
            project_id=project.project_id,
            is_smart=True,

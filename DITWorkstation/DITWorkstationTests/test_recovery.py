...
from DITWorkstation.Utils import normalize_path, now_local
...
def test_asset_soft_delete_and_restore_preserves_tags_and_audit(
    db_service, project, tmp_path
):
...
def test_project_soft_delete_and_restore_preserves_assets_and_logs(
    db_service, project, tmp_path
):
    ...
    db_service.add_media_asset(
        _asset(project.project_id, "asset-2", path, log_id=log.log_id)
    )
...
def test_recycle_bin_expiration_cleanup(db_service, project, tmp_path):
    ...
    cleaned = db_service.cleanup_expired_recycle_bin(now_local() + timedelta(seconds=1))

    assert deleted
    assert cleaned.affected_count == 1
    assert (
        db_service.restore_recycle_item(deleted.recovery_id).status
        is OperationStatus.NOT_FOUND
    )


def test_database_startup_cleans_expired_recycle_snapshots(
    db_service, project, tmp_path
):
...
def test_restore_all_recycle_items_restores_project_before_assets(
    db_service, project, tmp_path
):
    asset = _asset(
        project.project_id, "restore-all-asset", tmp_path / "restore-all.cr3"
    )
...
def test_empty_recycle_bin_permanently_removes_all_snapshots(
    db_service, project, tmp_path
):
...
def test_relink_preview_uses_relative_path_and_keeps_conflicts_unapplied(
    db_service, project, tmp_path
):
    ...
    assert db_service.get_media_asset("conflict").file_path == normalize_path(
        "/legacy/B/duplicate.cr3"
    )


def test_rename_rollback_requires_recorded_unmodified_files(
    db_service, project, tmp_path
):
    ...
    assert db_service.update_asset_path_by_old_path(
    ...
    assert db_service.get_media_asset("rename-asset").file_path == normalize_path(
        old_path
    )
    assert (
        service.rollback_rename(db_service, history.recovery_id).status

...
def test_recycle_bin_dialog_lists_and_restores_asset(
    db_service, project, tmp_path, monkeypatch
):
    asset = MediaAsset(
        asset_id="recycle-ui-asset",
        project_id=project.project_id,
        file_path=str(tmp_path / "asset.cr3"),
        file_name="asset.cr3",
        file_type=".cr3",
    )
...
def test_recycle_bin_dialog_restores_all_and_empties_items(
    db_service, project, tmp_path, monkeypatch
):
    assets = [
        MediaAsset(
            asset_id=f"recycle-ui-bulk-{index}",
            project_id=project.project_id,
            file_path=str(tmp_path / f"asset-{index}.cr3"),
            file_name=f"asset-{index}.cr3",
            file_type=".cr3",
        )
    ...
    assert dialog.table.rowCount() == 0
    assert all(
        db_service.get_media_asset(asset.asset_id) is not None for asset in assets
    )

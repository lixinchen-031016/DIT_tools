...
class FirstRunWizard(QWizard):
    ...
    def __init__(self, parent=None):
        ...
        # 页面顺序：欢迎 → 创建工作区 → 在工作区内创建项目 → 导入/备份/日志提示 → 完成
        self.addPage(_WelcomePage())
        self.addPage(_CreateWorkspacePage(self))
        self.addPage(_CreateProjectPage(self))
        self.addPage(_ImportGuidePage())
        self.addPage(_BackupGuidePage())
        self.addPage(_LogGuidePage())
...
class _CreateWorkspacePage(QWizardPage):
    ...
    def validatePage(self) -> bool:
        ...
        try:
            ...
            self._status_label.setText(f"✓ 已创建工作区：{name}")
            ...
            self._status_label.setText(f"✗ 创建失败：{e}")
...
class _CreateProjectPage(QWizardPage):
    ...
    def validatePage(self) -> bool:
        ...
        try:
            ...
            self._status_label.setText(f"✓ 已创建项目：{name}")
...
class _ImportGuidePage(QWizardPage):
    def __init__(self):
        super().__init__()
        self.setTitle("② 媒体导入")
        layout = QVBoxLayout(self)
        text = QLabel(
            "点击「下一步」后，将自动跳转到「媒体导入」视图。\n\n"
            "提示：\n"
...
class _BackupGuidePage(QWizardPage):
    def __init__(self):
        super().__init__()
        self.setTitle("③ 数据备份")
        layout = QVBoxLayout(self)
        text = QLabel(
            "导入完成后，请尽快去「数据备份」做存储卡的多目标安全备份。\n\n"
            "提示：\n"
            "  • 备份时记得在「关联项目」下拉选中刚创建的项目，\n"
            "    完成后会自动回写素材的备份位置（backup_locations）\n"
            "  • 建议至少 2 个备份目标（主盘 + 异地盘）\n"
            "  • 备份过程会校验源/目标的 xxhash64 一致性\n"
        )
        text.setWordWrap(True)
        layout.addWidget(text)


class _LogGuidePage(QWizardPage):
    def __init__(self):
        super().__init__()
        self.setTitle("④ 拍摄日志补录")
        layout = QVBoxLayout(self)
        text = QLabel(

...
class RenameRepository(BaseRepository):
    ...
    def get(self, rename_id: str) -> OperationResult:
        with self._connection() as conn:
            row = conn.execute(
                "SELECT * FROM rename_history WHERE rename_id = ?", (rename_id,)
            ).fetchone()
        if row is None:
            return OperationResult(
                OperationStatus.NOT_FOUND, f"重命名记录不存在: {rename_id}"
            )
        if row["reverted_at"]:
        ...
        try:
            with self._transaction() as conn:
                ...
                if cursor.rowcount != 1:
                    return OperationResult(
                        OperationStatus.CONFLICT, "重命名记录已回退或不存在"
                    )
                self._database._record_operation_in_transaction(
                    conn,
                    "回退文件重命名",
            ...
            return OperationResult(
                OperationStatus.SUCCESS, affected_count=1, recovery_id=rename_id
            )
        except sqlite3.Error as exc:
            return OperationResult(OperationStatus.ERROR, str(exc))
